...
The document didn't stay in the `docs/` folder. In May 2026, Johann [ported it into the WebMCP specification](https://github.com/webmachinelearning/webmcp/pull/181) as the Security and Privacy Considerations section, and if you open [the spec](https://webmachinelearning.github.io/webmcp/#security-privacy) today, the headings are still the three risks from issue #45, nearly word for word. I then wrote WebMCP's answers to the W3C's standard [security and privacy self-review questionnaire](https://w3c.github.io/security-questionnaire/), about thirty pointed questions on what the feature exposes and to whom, [which landed in June](https://github.com/webmachinelearning/webmcp/pull/195). It's dull work, and I think it's the most useful thing I've written for this group, because it's where we go on the record about what this API does and doesn't leak.

---

## The design questions I weighed in on

Once the threat model existed, most of my work turned into applying it to individual API decisions. A sample of the arguments, with receipts:
...
Other people build on the threat model too, which is the point of it being a living document. The spec now has an `untrustedContentHint`, [proposed by Julia](https://github.com/webmachinelearning/webmcp/issues/136), so a tool can flag output that contains user-generated content, which is the output injection part of the threat model turned into API surface. And when Chrome published its [security guidance for agent developers](https://developer.chrome.com/docs/agents/security) and [site developers](https://developer.chrome.com/docs/ai/webmcp/secure-tools) in June, it used the same vocabulary, the same nouns for the same threats. My document said up front that the spec can't mandate agent-side mitigations, only name the risks and who owns them, so seeing the guidance for those other layers line up with it is the outcome I was hoping for.
...
The honest reason I got into this is that I wanted to start looking at how WebMCP breaks so we can improve it. And I think getting more people onto it actually makes things safer, for the same scope reason: every task that moves onto a defined tool is a task that's no longer done by clicking through screenshots where nobody can see it. I still don't know all the ways it breaks. But there's a document now where we write them down as we find them, and that's a better place to be than where we started.
