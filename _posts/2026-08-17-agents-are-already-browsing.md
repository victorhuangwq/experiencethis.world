...
Other people build on the threat model too, which is the point of it being a living document. The spec now has an `untrustedContentHint`, [proposed by Julia Pagnucco](https://github.com/webmachinelearning/webmcp/issues/136), so a tool can flag output that contains user-generated content. That's the output injection part of the threat model turned into API surface. And in June, Chrome published [security guidance for agent developers](https://developer.chrome.com/docs/agents/security) and [for site developers writing tools](https://developer.chrome.com/docs/ai/webmcp/secure-tools) that picks up where a spec has to stop: token caps, origin restrictions, treating tools as state-changing by default, and hard character budgets for tool text that trace back to [an issue Johann filed in January](https://github.com/webmachinelearning/webmcp/issues/73). My document said up front that the spec can't mandate agent-side mitigations, only name the risks and who owns them. The part I like is that we all ended up using the same vocabulary, the same nouns for the same threats, across spec text, browser guidance and site guidance.
