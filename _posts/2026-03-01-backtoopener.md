...
Sometime last year I noticed something I'd been doing multiple times a day without thinking about it. I'd be deep in a conversation with an AI chat, it would link me somewhere, I'd read the page, and then I'd instinctively click the back button to get back to the chat. It just wasn't there: greyed out, dead. Then the little realization: right, back isn't available, this is a new tab. So I'd go find the X button instead. If the chat was the adjacent tab, which happens when the link is the newest thing I'd opened, closing dropped me right back into the conversation. Other times, when the tab bar was forty tabs deep and the chat wasn't next door, because I'd opened a pile of links or shuffled tabs around, I'd have to hunt for it, or just open a fresh chat and start over.
...
My first idea was a web platform API. Sites would opt in: add `rel="addOpenerToHistory"` to a link, or pass a flag to `window.open`, and links opened from your page would get a back button that returns to it. I wrote [an explainer](https://github.com/MicrosoftEdge/MSEdgeExplainers/blob/main/BackToOpener/explainer.md) and took it to [Domenic Denicola](https://github.com/domenic), a web standards engineer at Google in Tokyo. We'd worked together before; he gives honest feedback fast.
